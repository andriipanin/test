	...
	tinymce.create( 'tinymce.plugins.Wptuts', {
		...
		init: function ( ed, url ) {
			...
			ed.addCommand('custom_link', function() {
				...
				if (url !== null) {
					let return_text = '[link class="'+btn_class+'"]<a class="'+btn_class+'" href="'+url+'">'+btn_text+'</a>[/link]';
			...
			/****** add call us *******/
			ed.addButton('call_us', {
				title : 'Add call us phone',
				cmd : 'call_us',
				//image : url + '/moon.png'
				text: 'call us'
			});

			ed.addCommand('call_us', function() {
				let text      = prompt("Please add text");
				const phone      = prompt("Please add phone");
				const btn_text = prompt("Please add text to phone");

				text = ( text ) ? text : 'Call us';

				if (phone !== null) {
					let return_text = '[call_us class="call-us"]'+text+': <a class="" href="tel:'+phone+'">'+btn_text+'</a>[/call_us]';
					ed.execCommand('mceInsertContent', 0, return_text);
				}
			});
